...
class PlayState extends MusicBeatState
{
	...
	public var showRating:Bool = ClientPrefs.data.ratingPopups;
	public var showComboNum:Bool = ClientPrefs.data.comboNumPopups;
	public var showCombo:Bool = ClientPrefs.data.comboPopups;
	...
	override public function create()
	{
        ...
        msTxt.setFormat(Paths.font('vcr.ttf'), 24, 0xFF00FFFF, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		msTxt.x = 408 + 250;
		msTxt.y = 290 - 25;
...
{
	if (ClientPrefs.data.iconBounceType == 'Default' || ClientPrefs.data.iconBounceType == 'VS Steve' || ClientPrefs.data.iconBounceType == 'Os Engine' || ClientPrefs.data.iconBounceType == 'SB Engine' || ClientPrefs.data.iconBounceType == 'Lore Engine') {
	...
	if (ClientPrefs.data.iconBounceType == 'Furret Engine') {
		iconP1.setGraphicSize(Std.int(FlxMath.lerp(150, iconP1.width, 0.5 / playbackRate)));
		iconP2.setGraphicSize(Std.int(FlxMath.lerp(150, iconP2.width, 0.5 / playbackRate)));
	}
		
	if (ClientPrefs.data.iconBounceType == 'Shadow Engine') {
	...
	if (ClientPrefs.data.iconBounceType == 'Definitive Edition') {
		iconP1.setGraphicSize(Std.int(FlxMath.lerp(150, iconP1.width, 0.8)),Std.int(FlxMath.lerp(150, iconP1.height, 0.8 / playbackRate)));
		iconP2.setGraphicSize(Std.int(FlxMath.lerp(150, iconP2.width, 0.8)),Std.int(FlxMath.lerp(150, iconP2.height, 0.8 / playbackRate)));
	}
		
	if (ClientPrefs.data.iconBounceType == 'SB Engine') {
		if (iconP1.angle >= 0) {
			if (iconP1.angle != 0) {
...
function judgeNote(note:Note = null, ?miss:Bool = false, scoreBop:Bool = true)
{
	if (ClientPrefs.data.vsliceLegacyBar)
		scoreBop = false;
...
private function popUpScore(note:Note = null, ?miss:Bool = false):Void
{
	if (ClientPrefs.data.showMS && !ClientPrefs.data.hideHud) {
			...
			FlxTween.tween(msTxt,
				...
				{onComplete: function(_) {
						...
						FlxTween.tween(msTxt, {alpha: 0}, 0.1 / playbackRate, {
							// ease: FlxEase.circOut,
							onComplete: function(_){msTxt.visible = false;},
							startDelay: 0.25 / playbackRate
				});
			}
		});
	}
	...
	if (showRating) {
		...
		rating.setupRating(uiPrefix + daRating.image + uiPostfix);
		rating.ratingOtherStuff();
		popUpGroup.insert(0, rating);
	}

	if (showCombo && combo >= 50) {
		comboPop = popUpGroup.recycle(Popup);
		comboPop.setupCombo(uiPrefix + 'combo' + uiPostfix);
		comboPop.comboOtherStuff();
		popUpGroup.insert(0, comboPop);
	}
	...
	if (showComboNum)
	{
		...
		while(tempCombo >= 10)
		{
			...
			for (daLoop=>i in separatedScore)
			{
				...
				numScore.setupNumber(uiPrefix + 'num' + i + uiPostfix, daLoop, tempComboAlt);
				numScore.numberOtherStuff();
				popUpGroup.insert(0, numScore);
			}

	if (showRating) popUpGroup.sort((o, a, b) ->
				{
...
public function goodNoteHit(note:Note):Void
{
	...
	if (!note.hitCausesMiss) // Common notes
	{
		...
		if (!note.isSustainNote)
		{
			...
			if (!ClientPrefs.data.lessBotLag && showPopups) popUpScore(note);
			else judgeNote(note);
...
{
	if (ClientPrefs.data.iconBounceType == 'Os Engine') {
	...
	if (ClientPrefs.data.iconBounceType == 'Dave and Bambi') {
		final funny:Float = Math.max(Math.min(healthBar.valueFunction(),(maxHealth*0.95)),0.1);
	...
	if (ClientPrefs.data.iconBounceType == 'Old Psych' || ClientPrefs.data.iconBounceType == 'Shadow Engine') {
		iconP1.setGraphicSize(Std.int(iconP1.width + 30));
		iconP2.setGraphicSize(Std.int(iconP2.width + 30));
	}

	if (ClientPrefs.data.iconBounceType == 'Lore Engine') {
	...
	if (ClientPrefs.data.iconBounceType == 'Default' || ClientPrefs.data.iconBounceType == 'Os Engine') {
